...
const platforms = [
  {
    name: 'Spotify',
    desc: 'Stream our full catalog in high quality',
  ...
  {
    name: 'SoundCloud',
    desc: 'Demos, bootlegs and exclusive mixes',
  ...
  {
    name: 'YouTube',
    desc: 'Visual releases, sets and sessions',
...
export function PlatformsSection() {
  return (
    <section
      id="platforms"
      className="relative px-5 md:px-10 py-28 md:py-40 max-w-2xl mx-auto"
      aria-label="Streaming platforms"
    >
      {/* Section label */}
      <motion.p
        className="text-[10px] tracking-[0.45em] text-[rgba(245,245,245,0.3)] uppercase mb-10"
        initial={{ opacity: 0 }}
      ...
      >
        {platforms.map((p) => (
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            className="group flex items-center justify-between px-6 py-5 rounded-2xl cursor-pointer"
            style={{
            ...
            onMouseEnter={(e) => {
              ...
              el.style.borderColor = 'rgba(47,109,255,0.15)'
            }}
            onMouseLeave={(e) => {
              const el = e.currentTarget
              el.style.background = 'rgba(11,11,11,0.7)'
              el.style.borderColor = 'rgba(245,245,245,0.06)'
            }}
          >
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className="text-[#F5F5F5] font-light text-base tracking-tight">{p.name}</p>
                <p className="text-xs text-[rgba(245,245,245,0.35)] mt-0.5 font-light">{p.desc}</p>
              </div>
            </div>
            <svg
              className="w-4 h-4 text-[rgba(245,245,245,0.2)] group-hover:text-[#2F6DFF] transition-all duration-300 group-hover:translate-x-1"
